...
for(i in 1:20){  
  ...
  simulated_paths_IS <-  sim_sobol_paths(S0, r, sigma, capT, nSteps, nPaths=2^18)
  LSM_IS <- Price_Amr_put_LSM(S0, K, r, sigma, capT, nSteps, nPaths = 2^18, stock_paths = simulated_paths_IS)
  Delta_LSM_IS <- Price_Amr_put_DeltaLSM(S0, K, r, sigma, capT, nSteps, nPaths = 2^18, stock_paths = simulated_paths_IS)
...
table2 <- cbind.data.frame(S = S0, sigma = sigma, T = capT, CRR, Simulated_American)
